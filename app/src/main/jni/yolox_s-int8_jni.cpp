...
generate_yolox_proposals(std::vector<GridAndStride> grid_strides, const ncnn::Mat &feat_blob,
    ...
    for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++) {
        ...
        float x_center = (feat_ptr[0 * num_grid] + grid0) * stride;
        float y_center = (feat_ptr[1 * num_grid] + grid1) * stride;
        float w = exp(feat_ptr[2 * num_grid]) * stride;
        float h = exp(feat_ptr[3 * num_grid]) * stride;
        float x0 = x_center - w * 0.5f;
        float y0 = y_center - h * 0.5f;

        float box_objectness = feat_ptr[4 * num_grid];
        for (int class_idx = 0; class_idx < num_class; class_idx++) {
            float box_cls_score = feat_ptr[(5 + class_idx) * num_grid];
...
Java_com_ironsublimate_ncnn_1objectdetection_YoloxInt8_Init(JNIEnv *env, jobject thiz,
    ...
    yoloXInt8.register_custom_layer("YoloV5Focus", YoloV5Focus_layer_creator);

    // init param
    //this model is converted by pnnx and ncnn Quantization, yolox_s is converted from onnx
    //yolox_s need about 350ms yolox_s-int8 need about 234 ms
    {
        int ret = yoloXInt8.load_param(mgr, "yolox_s-int8.param");
        if (ret != 0) {
    ...
    // init bin
    {
        int ret = yoloXInt8.load_model(mgr, "yolox_s-int8.bin");
        if (ret != 0) {
...
Java_com_ironsublimate_ncnn_1objectdetection_YoloxInt8_Detect(JNIEnv *env, jobject thiz,
                                                              jobject bitmap,
                                                              jboolean use_gpu) {
...
//    in_pad.substract_mean_normalize(mean_vals, norm_vals);
    ...
    {
        ncnn::Extractor ex = yoloXInt8.create_extractor();
...
JNIEXPORT jboolean JNICALL
Java_com_ironsublimate_ncnn_1objectdetection_YoloxInt8_Deinit(JNIEnv *env, jobject thiz) {
    yoloXInt8.clear();
    return JNI_TRUE;
}
}
